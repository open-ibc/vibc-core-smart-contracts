...
contract UniversalChannelTest is Base {
        ...
        for (uint256 i = 0; i < orders.length; i++) {
            for (uint256 j = 0; j < feesEnabled.length; j++) {
                ...
                ChannelSetting memory setting = ChannelSetting(
                    orders[i],
                    "1.0",
                    IbcUtils.addressToPortId("polyibc.eth1.", address(eth1.ucHandler())),
                ...
                ChannelSetting memory setting2 = ChannelSetting(
                    setting.ordering,
                    setting.version,
                    IbcUtils.addressToPortId("polyibc.eth2.", address(eth2.ucHandler())),
                    setting.channelId,
                    setting.feeEnabled,
                    validProof
                );
    ...
    function assert_channel(
        ...
        Channel memory channel2Expected =
            vc1.expectedChannel(address(vc1.ucHandler()), channelId1, vc2.getConnectionHops(), setting2);
